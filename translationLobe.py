...
import unicodedata
import json
# from app.record.sql_manager import SQLManager

"""
Hebrew Translation Engine

This module loads pre-processed Excel files (milon_zachar_nekeva_new.xlsx) that have been
prepared by load_milon.py. The preprocessing script extracts first words from multi-word
phrases and adds them as "Special" entries, eliminating the need for runtime preprocessing.

Key Dependencies:
- load_milon.py: Preprocesses milon_zachar_nekeva.xlsx → milon_zachar_nekeva_new.xlsx
- The "Special" column marks auto-generated first-word entries
"""

# === CONSOLIDATED HEBREW CONSTANTS ===
# All Hebrew language constants in one organized structure to eliminate duplicates
HEBREW_CONSTANTS = {
    'UNITS': {
    ...
    'CONSTRUCT_UNITS': {
        3: 'שְׁלוֹשֶׁת', 4: 'אַרְבַּעַת', 5: 'חֲמֵשֶׁת', 6: 'שֵׁשֶׁת',
        7: 'שִׁבְעַת', 8: 'שְׁמוֹנַת', 9: 'תִּשְׁעַת', 10: 'עֲשֶׂרֶת'
    },
    'CONSTRUCT_UNITS_LOW': {
        'm': {1: 'אַחַד', 2: 'שְׁנֵי'},   # Same as absolute for masculine 1, construct for 2
        'f': {1: 'אַחַת', 2: 'שְׁתֵי'}    # Same as absolute for feminine 1, construct for 2
    },
    'ORDINAL_MASCULINE': {
        1: 'ראשון', 2: 'שֵנִי', 3: 'שלישִּי', 4: 'רביעי', 5: 'חמישִּי',
        6: 'שישִּי', 7: 'שביעי', 8: 'שמינִּי', 9: 'תשיעי', 10: 'עשירי'
    },
    'ORDINAL_FEMININE': {
...
# Note: HEBREW_ORDINAL_MASCULINE and HEBREW_ORDINAL_FEMININE are now centralized in HEBREW_CONSTANTS above
...
PERSON_INDICATORS = {
    ...
    'second': {
        ...
        'female': {'אַתְּ', 'שאַתְּ', 'ואַתְּ', 'כשאַתְּ', 'שאת/ה'},  # Include dual gender form שאת/ה as female person indicator
        'neutral': {'שלך', 'איתך', 'לך', 'אותך'}
    }
}
...
class TextProcessingTable:
    """Table-based text processor for Hebrew text processing"""
    ...
    def __init__(self, text, debug=False):
        ...
        for line_num, line in enumerate(lines):
            ...
            for word in words:
                self.rows.append({
                    'row': word_index,
                    'source': word,              # Column A: Original text
                    'gender': None,              # Column B: Gender context (which gender form to use)
                    'gender_source': None,       # Column C: Source of gender information (Milon/Noun_genders/etc)
                    'person': None,              # Column D: Person context (1st/2nd person)
                    'milon': None,               # Column E: Dictionary conversion
                    'heb2num': None,             # Column F: Number conversion
                    'pattern': None,             # Column G: Pattern conversion
                    'consumed': None,            # Consumption tracking
                    'processor': None,           # Which processor handled it
                    'notes': '',                 # Debug information
                    'line_num': line_num + 1     # Track original line number
                })
                word_index += 1

            # Store line boundary information
            if words:  # Only if line had words
    ...
    def mark_consumed(self, start_row, end_row, reason, processor_name):
        for i in range(start_row, end_row + 1):
            row_index = i - 1
            if 0 <= row_index < len(self.rows):
                self.rows[row_index]['consumed'] = 'YES'
                self.rows[row_index]['processor'] = processor_name

    def is_consumed(self, row_number):
        row_index = row_number - 1
        return 0 <= row_index < len(self.rows) and self.rows[row_index]['consumed'] == 'YES'

    def set_result(self, row_number, column, value, processor_name):
        row_index = row_number - 1
        if 0 <= row_index < len(self.rows):
            self.rows[row_index][column] = value
            self.rows[row_index]['processor'] = processor_name

            # Debug: Show the row state after modification
            if self.debug:
                row = self.rows[row_index]
                print(f"[TABLE_DEBUG] Row {row_number} after {processor_name}: source='{row.get('source', '')}' milon='{row.get('milon', '')}' heb2num='{row.get('heb2num', '')}' pattern='{row.get('pattern', '')}'")
        else:
            if self.debug:
                print(f"[TABLE_DEBUG] Warning: Row {row_number} is out of range (1-{len(self.rows)})")

    def get_final_result(self):
        """Get final result using priority: E→D→C→B→A (rightmost column with data wins), preserving line breaks"""
        if not hasattr(self, 'line_boundaries') or not self.line_boundaries:
            # Fallback to old behavior if no line boundaries
            return self._get_final_result_single_line()

        result_lines = []

        for line_boundary in self.line_boundaries:
            line_words = []
            start_row = line_boundary['start_row']
            end_row = line_boundary['end_row']

            for row_num in range(start_row, end_row + 1):
                row = self.rows[row_num - 1]  # Convert to 0-based index

                # If ANY column has a span marker (like "2", "3"), skip this ENTIRE row
                has_span_marker = (
                    (row.get('pattern', '') and row['pattern'].isdigit() and len(row['pattern']) == 1) or
                    (row.get('heb2num', '') and row['heb2num'].isdigit() and len(row['heb2num']) == 1) or
                    (row.get('milon', '') and row['milon'].isdigit() and len(row['milon']) == 1) or
                    (row.get('gender', '') and row['gender'].isdigit() and len(row['gender']) == 1)
                )

                if has_span_marker:
                    continue  # Skip this entire row

                # Priority from right to left: pattern → heb2num → milon → source (gender and person columns are metadata only)
                if row.get('pattern') and row['pattern'] not in ['None', '', None]:  # Column F
                    line_words.append(f"~{row['pattern']}~")
                elif row.get('heb2num') and row['heb2num'] not in ['None', '', None]:  # Column E
                    line_words.append(f"~{row['heb2num']}~")
                elif row.get('milon') and row['milon'] not in ['None', '', None]:  # Column D
                    line_words.append(f"~{row['milon']}~")
                else:  # Column A - always include source word (gender and person columns are metadata only)
                    line_words.append(row['source'])
    ...
    def _get_final_result_single_line(self):
        """Fallback method for single-line processing (old behavior)"""
        result_words = []
        for row in self.rows:
            ...
            has_span_marker = (
                ...
                (row.get('heb2num', '') and row['heb2num'].isdigit() and len(row['heb2num']) == 1) or
                (row.get('milon', '') and row['milon'].isdigit() and len(row['milon']) == 1) or
                (row.get('gender', '') and row['gender'].isdigit() and len(row['gender']) == 1) or
                (row.get('person', '') and row['person'].isdigit() and len(row['person']) == 1)
            )
            ...
            # Priority from right to left: pattern → heb2num → milon → source (gender and person columns are metadata only)
            if row.get('pattern') and row['pattern'] not in ['None', '', None]:  # Column F
                result_words.append(f"~{row['pattern']}~")
            elif row.get('heb2num') and row['heb2num'] not in ['None', '', None]:  # Column E
                result_words.append(f"~{row['heb2num']}~")
            elif row.get('milon') and row['milon'] not in ['None', '', None]:  # Column D
                result_words.append(f"~{row['milon']}~")
            else:  # Column A - always include source word (gender and person columns are metadata only)
                result_words.append(row['source'])
        return ' '.join(result_words)

    def export_to_excel(self, filename="hamara_table.xlsx", title="Processing Table", line_number=None):
        """Export the complete table to Excel file"""
        try:
            import pandas as pd
            from datetime import datetime

            # Add line number to filename if provided
            if line_number is not None:
                # Extract base name and extension
                if '.' in filename:
                    base_name, extension = filename.rsplit('.', 1)
                    filename = f"{base_name}{line_number}.{extension}"
                else:
                    filename = f"{filename}{line_number}"
            ...
            if os.path.exists(filename):
                try:
                    os.remove(filename)
                    if line_number is not None:
                        print(f"[FILE] Deleted existing {filename}")
            ...
            for row in self.rows:
                export_data.append({
                    ...
                    'Gender_Source': row['gender_source'] or '',
                    'Person': row['person'] or '',
                    'Milon': row['milon'] or '',
                    'Heb2num': row['heb2num'] or '',
                    'Pattern': row['pattern'] or '',
                    'Consumed': row['consumed'] or '',
            ...
            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                # Main table sheet
                df.to_excel(writer, sheet_name='Processing Table', index=False)
                ...
                summary_data = {
                    ...
                    'Value': [
                        len(self.rows),
                        len([r for r in self.rows if r['milon']]),
                        len([r for r in self.rows if r['heb2num']]),
                        len([r for r in self.rows if r['pattern']]),
                        len([r for r in self.rows if r['consumed']]),
                ...
                table_sheet = writer.sheets['Processing Table']
                summary_sheet = writer.sheets['Summary']

                # Auto-adjust column widths for table sheet
    ...
    def display_table(self, title="Processing Table", line_number=None):
        """Display basic table info and export to Excel"""
        if hasattr(self, 'finalize_processing_times'):
            self.finalize_processing_times()  # Ensure all times are finalized
        print(f"[TABLE] {title} - {len(self.rows)} rows processed")
        self.export_to_excel("hamara_table.xlsx", title, line_number)

    def finalize_processing_times(self):
        """Finalize processing times - placeholder method"""
        # This method can be implemented if timing functionality is needed
        pass
...
class ConsolidatedDateProcessor:
    """Unified date processing to eliminate duplicate patterns"""
    ...
    def convert_day_to_hebrew(self, day_num):
        """Convert day number to Hebrew (ordinals 1-10, cardinals 11+)"""
        if 1 <= day_num <= 10:
            return self.text_processor.ORDINAL_MASCULINE[day_num]
        else:
            return self.heb2num(day_num, 'm')

    def convert_month_to_hebrew(self, month_num):
        """Convert month number to Hebrew month name"""
        return self._hebrew_months.get(month_num, str(month_num))

    def convert_year_to_hebrew(self, year_num):
        """Convert year number to Hebrew (always feminine)"""
        return self.heb2num(year_num, 'f', is_year=True)

    def validate_date_components(self, day, month, year=None):
        """Validate date components are in valid ranges"""
        day_valid = 1 <= day <= 31
        month_valid = 1 <= month <= 12
        year_valid = year is None or (1900 <= year <= 2100)
        return day_valid and month_valid and year_valid

    def format_hebrew_date(self, day_text, month_name, year_text=None, prefix=""):
        """Format Hebrew date string consistently"""
        if year_text:
            return f"{prefix}{day_text} ב{month_name} {year_text}"
        else:
            return f"{prefix}{day_text} ב{month_name}"

    def convert_date_pattern(self, day_str, month_str, year_str=None, prefix=""):
        """Unified date conversion method"""
        try:
            day_num = int(day_str)
            month_num = int(month_str)
            year_num = int(year_str) if year_str else None

            if not self.validate_date_components(day_num, month_num, year_num):
                return None

            day_text = self.convert_day_to_hebrew(day_num)
            month_name = self.convert_month_to_hebrew(month_num)
            year_text = self.convert_year_to_hebrew(year_num) if year_num else None

            return self.format_hebrew_date(day_text, month_name, year_text, prefix)
    ...
    def _process_percentage_patterns_in_table(self, table):
        ...
        for i, row in enumerate(table.rows, 1):
            ...
            if match:
                ...
                converted = f"{prefix} {hebrew_percentage}"
